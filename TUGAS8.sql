...
select mp.NIM, mp.NAMA, mp.JENIS_KELAMIN, mp.KOTA_LAHIR, mp.ANGKATAN
    from(
        select *from AKADEMIK.MAHASISWA_PINDAHAN mp1
            where substr(mp1.KOTA_LAHIR, 1, 1) = 'B'
        union
        select *from AKADEMIK.MAHASISWA_PINDAHAN mp2
...
from AKADEMIK.mahasiswa m, (
        select * from akademik.mahasiswa_pindahan mp 
        where mp.NAMA like "%budi%"
    ) mp_budi
where m.KOTA_LAHIR = mp_budi.KOTA_LAHIR;

# No 2
select m.NIM, m.NAMA, m.ANGKATAN, m.KOTA_LAHIR, "memiliki kota lahir yang sama dengan", mp.NAMA, mp.KOTA_LAHIR
from AKADEMIK.mahasiswa m, (
